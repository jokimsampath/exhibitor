...
if (isset($postdata) && !empty($postdata)) {
    ...
    if ($method == 'POST') {
        $str = '';
        foreach ($data as $obj) {
            $str = $str."('$user_id', '$obj->standPersonName', '$obj->email', '$obj->phone', '$obj->companyName', '$obj->country', '1'),";
        }
        $str = rtrim($str, ',');
        
        $sql1 = "INSERT INTO vipbadge (user_id, standPersonName, email, phone, companyName, country, status) VALUES $str";
        echo $sql1;
...
function getData()
{
    ...
    $rows = [];
    $sql = "SELECT * FROM vipbadge where user_id='$user_id' and status='1'";
