...
if (isset($postdata) && !empty($postdata)) {
    ...
    $data = $request->data;
    if ($method == 'POST') {
        $typeOfSpace = $data[0]->typeOfSpace;
        $phaseType = $data[0]->phaseType;
        $quantity = $data[0]->quantity;
        $rate = $data[0]->rate;
        global $dataRows;
        $dataRows = getData();
        if (sizeof($dataRows) > 0) {
            $sql = "UPDATE powerrequirementexhibition SET user_id = '$user_id', typeOfSpace = '$typeOfSpace', phaseType = '$phaseType', quantity = '$quantity', rate = '$rate', status = '1'";
            ...
            $sql1 = "INSERT INTO powerrequirementexhibition (user_id, typeOfSpace, phaseType, quantity, rate, status) VALUES ('$user_id', '$typeOfSpace', '$phaseType', $quantity, '$rate', true)";
...
function getData()
{
    ...
    $rows = [];
    $sql = "SELECT * FROM powerrequirementexhibition where user_id='$user_id' and status='1'";
    if ($result = mysqli_query($mysqli, $sql)) {
