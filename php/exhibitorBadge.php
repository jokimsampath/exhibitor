...
if (isset($postdata) && !empty($postdata)) {
    ...
    if ($method == 'POST') {
        $str = '';
        foreach ($data as $obj) {
            $str = $str."('$user_id', '$obj->standPersonName', '$obj->jobTitle', '$obj->companyName', '$obj->country', true),";
        }
        $str = rtrim($str, ',');
        $sql1 = "INSERT INTO exhibitorbadge (user_id, standPersonName, jobTitle, companyName, country, status) VALUES $str";
...
function getData()
{
    ...
    $rows = [];
    $sql = "SELECT * FROM exhibitorbadge where user_id='$user_id' and status='1'";
