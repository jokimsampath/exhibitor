...
getData() {
  ...
  this.webService.postApi(UrlConstants.EXHIBITOR_BADGE, ExhibitorBadgeReqModel).subscribe((res: any) => {
...
processData() {
  ...
  for (let i = 0; i < 30; i++) {
    let exhibitorBadgeModel = JSON.parse(JSON.stringify(this.exhibitorBadgeModel));
...
onSubmit() {
  ...
  this.exhibitorBadgeModelArr.forEach(element => {
    if (element.standPersonName !== "" && element.jobTitle !== "" && element.companyName !== "" && element.country !== "" && element.status == '0') {
