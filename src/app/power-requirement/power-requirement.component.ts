...
import {
  PowerRequirementTemporaryModel,
  PowerRequirementExhibitionModel,
  PowerRequirementTemporaryDetailsModel,
  PowerRequirementExhibitionDetailsModel,
  PowerRequirementTemporaryRequestModel,
  PowerRequirementExhibitionRequestModel,
...
export class PowerRequirementComponent implements OnInit {
  ...
  PowerRequirementTemporaryForm: Array<object> =
    PowerRequirementConstants.powerRequirementTemporary;
  PowerRequirementExhibitionForm: Array<object> =
    PowerRequirementConstants.powerRequirementExhibiton;
  ...
  PowerRequirementTemporaryRequest: PowerRequirementTemporaryRequestModel =
    new PowerRequirementTemporaryRequestModel();
  PowerRequirementExhibitionRequest: PowerRequirementExhibitionRequestModel =
    new PowerRequirementExhibitionRequestModel();
  ...
  getData() {
    ...
    this.webService
      .postApi(
        UrlConstants.POWER_REQUIREMENT_TEMPORARY,
        PowerRequirementReqModel
      )
      .subscribe((res: any) => {
        ...
        this.webService
          .postApi(
            UrlConstants.POWER_REQUIREMENT_EXHIBITION,
            PowerRequirementReqModel
          )
          .subscribe((res: any) => {
            this.preloaderService.hide();
  ...
  processData() {
    if (
      this.PowerRequirementTemporaryArrCopy.length == 0 &&
      this.PowerRequirementExhibitionArrCopy.length == 0
    ) {
      this.notificationService.danger(
        "No power requirements found, plese fill power requirement to proceed to next step 'Exhibitor Badge'"
      );
    }
    ...
    this.PowerRequirementTemporaryForm.forEach((element, i) => {
      ...
      this.PowerRequirementTemporaryDetailsModel =
        new PowerRequirementTemporaryDetailsModel();
      ...
      this.PowerRequirementTemporaryDetailsModel.chargesPerKwPerDay =
        element['chargesPerKwPerDay'];
      this.PowerRequirementTemporaryDetailsModel.code = element['code'];
      if (this.PowerRequirementTemporaryArrCopy[i]) {
        this.PowerRequirementTemporaryDetailsModel.quantity =
          this.PowerRequirementTemporaryArrCopy[i]['quantity'];
        this.PowerRequirementTemporaryDetailsModel.numberOfDays =
          this.PowerRequirementTemporaryArrCopy[i]['numberOfDays'];
        this.PowerRequirementTemporaryDetailsModel.rate =
          this.PowerRequirementTemporaryArrCopy[i]['rate'];
      } else {
        this.PowerRequirementTemporaryDetailsModel.numberOfDays =
      ...
      this.PowerRequirementTemporaryArr.push(
        this.PowerRequirementTemporaryDetailsModel
      );
    });
    this.PowerRequirementExhibitionForm.forEach((element, i) => {
      this.PowerRequirementExhibitionDetailsModel = null;
      this.PowerRequirementExhibitionDetailsModel =
        new PowerRequirementExhibitionDetailsModel();
      let property = element['code'];
      let tempObj = {};
      ...
      this.PowerRequirementExhibitionDetailsModel.chargesPerKw4Day =
        element['chargesPerKw4Day'];
      if (this.PowerRequirementExhibitionArrCopy[i]) {
        this.PowerRequirementExhibitionDetailsModel.typeOfSpace =
          this.PowerRequirementExhibitionArrCopy[i]['typeOfSpace'];
        this.PowerRequirementExhibitionDetailsModel.phaseType =
          this.PowerRequirementExhibitionArrCopy[i]['phaseType'];
        this.PowerRequirementExhibitionDetailsModel.quantity =
          this.PowerRequirementExhibitionArrCopy[i]['quantity'];
        this.PowerRequirementExhibitionDetailsModel.quantity =
          this.PowerRequirementExhibitionArrCopy[i]['quantity'];
        this.PowerRequirementExhibitionDetailsModel.rate =
          this.PowerRequirementExhibitionArrCopy[i]['rate'];
      } else {
        this.PowerRequirementExhibitionDetailsModel.typeOfSpace =
          element['typeOfSpace'];
        this.PowerRequirementExhibitionDetailsModel.phaseType =
      ...
      this.PowerRequirementExhibitionArr.push(
        this.PowerRequirementExhibitionDetailsModel
      );
    });
  }
  ...
  modelChanged(event, formName, i) {
    ...
    if (formName == 'powerRequirementTemporaryForm') {
      ...
      chargesPerDay =
        this.PowerRequirementTemporaryArr[i]['chargesPerKwPerDay'];
      if (qty > 0 && chargesPerDay > 0) {
        this.PowerRequirementTemporaryArr[i]['rate'] =
          (Number(chargesPerDay) + Number(chargesPerDay) * Number(this.GST)) *
          Number(qty) *
          Number(totalDays);
      } else {
      ...
      if (qty > 0 && chargesPerDay > 0) {
        this.PowerRequirementExhibitionArr[i]['rate'] =
          (Number(chargesPerDay) + Number(chargesPerDay) * Number(this.GST)) *
          Number(qty);
      } else {
  ...
  onSubmitPowerRequirementTemporary() {
    if (this.PowerRequirementTemporaryArr[0]['quantity'] < 1) {
      this.notificationService.danger(
        'Quantity in KW should be greater than 1'
      );
      return;
    }
    ...
    this.webService
      .postApi(UrlConstants.POWER_REQUIREMENT_TEMPORARY, PowerRequirementModel)
      .subscribe((res: any) => {
        this.preloaderService.hide();
        if (res.STATUS == 'OK') {
  ...
  onSubmitPowerRequirementExhibition() {
    ...
    this.notificationService.success('Details updated successfully');
    this.webService
      .postApi(UrlConstants.POWER_REQUIREMENT_EXHIBITION, PowerRequirementModel)
      .subscribe((res: any) => {
        this.preloaderService.hide();
        if (res.STATUS == 'OK') {
          this.notificationService.success('Details updated successfully');
